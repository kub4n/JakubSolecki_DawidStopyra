    ...
    public function extendWith(\Maatwebsite\Sidebar\Menu $menu)
    {
        $menu->group(trans('core::sidebar.content'), function (Group $group) {
            $group->item(trans('blog::blog.title'), function (Item $item) {
                ...
                $item->item(trans('blog::reservation.title.reservation'), function (Item $item) {
                    $item->weight(1);
                    $item->route('admin.blog.reservation.index');
                    $item->authorize(
                        $this->auth->hasAccess('blog.reservations.index')
                    );
                });
                $item->authorize(
